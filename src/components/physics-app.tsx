...
function StatusBadge({ children, tone = "default" }: { children: React.ReactNode; tone?: "default" | "success" | "warning" | "danger" | "info" }) {
  const styles = { default: "border-slate-200 bg-slate-50 text-slate-700", success: "border-emerald-200 bg-emerald-50 text-emerald-700", warning: "border-amber-200 bg-amber-50 text-amber-700", danger: "border-rose-200 bg-rose-50 text-rose-700", info: "border-blue-200 bg-blue-50 text-blue-700" }
...
function MetricCard({ label, value, hint, icon: Icon, tone = "blue" }: { label: string; value: string; hint: string; icon: React.ElementType; tone?: "blue" | "green" | "amber" | "red" }) {
  const toneStyles = { blue: "bg-blue-50 text-blue-700", green: "bg-emerald-50 text-emerald-700", amber: "bg-amber-50 text-amber-700", red: "bg-rose-50 text-rose-700" }
...
function LoginScreen({ onLogin }: { onLogin: (role: UserRole, displayName: string) => void }) {
  ...
  return <main className="relative flex min-h-[100dvh] items-center justify-center overflow-hidden bg-slate-900 px-4 py-8 md:px-8" style={{ backgroundImage: "url('/assets/teaching-login-bg.jpg')", backgroundPosition: "center", backgroundSize: "cover" }}>
    <div className="absolute inset-0 bg-[#081a31]/55" />
    <div className="absolute inset-0 bg-[linear-gradient(90deg,rgba(8,26,49,.82),rgba(8,26,49,.35)_55%,rgba(8,26,49,.48))]" />
    <div className="relative z-10 grid w-full max-w-6xl gap-8 lg:grid-cols-[1.15fr_420px] lg:items-center">
      <section className="hidden text-white lg:block"><div className="mb-10 max-w-xl"><p className="mb-5 text-xs font-semibold uppercase tracking-[0.18em] text-blue-100/85">长江大学文理学院</p><h1 className="text-5xl font-semibold leading-[1.1] tracking-[-0.04em]">大学物理<br /><span className="text-blue-100">AI 智慧教学系统</span></h1><p className="mt-6 max-w-lg text-base leading-7 text-blue-50/85">围绕课程学习、项目实践和过程评价，建立从诊断到反馈的完整学习路径。</p></div><div className="grid max-w-xl grid-cols-3 gap-3"><div className="rounded-lg border border-white/15 bg-white/10 p-4 backdrop-blur-sm"><Network className="mb-5 size-5 text-blue-100" /><p className="text-sm font-medium">知识图谱</p><p className="mt-1 text-xs leading-5 text-blue-50/70">按章节连接概念与项目</p></div><div className="rounded-lg border border-white/15 bg-white/10 p-4 backdrop-blur-sm"><FlaskConical className="mb-5 size-5 text-blue-100" /><p className="text-sm font-medium">虚拟实验</p><p className="mt-1 text-xs leading-5 text-blue-50/70">调节参数查看测量变化</p></div><div className="rounded-lg border border-white/15 bg-white/10 p-4 backdrop-blur-sm"><FileBarChart className="mb-5 size-5 text-blue-100" /><p className="text-sm font-medium">过程评价</p><p className="mt-1 text-xs leading-5 text-blue-50/70">保留任务和实验证据</p></div></div></section>
...
function OverviewView({ state, navigate }: { state: AppState; navigate: (view: StudentView) => void }) {
  ...
  return <div className="space-y-6"><div className="grid gap-4 xl:grid-cols-[1.4fr_1fr]"><Card className="overflow-hidden rounded-lg border-0 bg-[var(--sidebar)] text-white shadow-none"><CardContent className="relative p-6 md:p-8"><div className="absolute right-0 top-0 size-52 translate-x-1/4 -translate-y-1/4 rounded-full border-[28px] border-white/5" /><p className="text-xs font-semibold uppercase tracking-[0.16em] text-blue-100/75">今天的学习焦点</p><h2 className="mt-4 max-w-xl text-2xl font-semibold leading-tight tracking-[-0.03em] md:text-3xl">把一个物理问题，推进到下一条可验证证据。</h2><p className="mt-4 max-w-xl text-sm leading-6 text-blue-50/75">先诊断，再匹配；先留下过程，再生成评价。每一步都能回到课程章节和项目目标。</p><div className="mt-7 flex flex-wrap gap-3"><Button className="bg-white text-[var(--primary-strong)] hover:bg-blue-50" onClick={() => navigate(state.diagnosis ? "projects" : "diagnosis")}>{state.diagnosis ? "查看项目匹配" : "开始学情诊断"}<ArrowRight className="size-4" /></Button><Button variant="outline" className="border-white/25 bg-transparent text-white hover:bg-white/10 hover:text-white" onClick={() => navigate("curriculum")}>浏览课程结构</Button></div></CardContent></Card><Card className="rounded-lg border-[var(--border)] shadow-none"><CardHeader className="p-6 pb-3"><div className="flex items-center justify-between"><div><CardTitle className="text-base">学习路径</CardTitle><CardDescription className="mt-1">当前学习记录的完成情况</CardDescription></div><StatusBadge tone={progress >= 60 ? "success" : "info"}>{progress}%</StatusBadge></div></CardHeader><CardContent className="p-6 pt-3"><Progress value={Math.max(progress, diagnosisScore ? 20 : 0)} className="h-2" /><div className="mt-5 space-y-4">{steps.map((step, index) => <button key={step.label} className="flex w-full items-start gap-3 text-left" onClick={() => navigate((index === 0 ? "diagnosis" : index === 1 ? "projects" : index === 2 ? "tasks" : index === 3 ? "lab" : "report"))}><span className={cn("mt-0.5 grid size-5 shrink-0 place-items-center rounded-full border text-[11px]", step.done ? "border-emerald-200 bg-emerald-50 text-emerald-700" : "border-slate-200 bg-slate-50 text-slate-500")}>{step.done ? <Check className="size-3" /> : index + 1}</span><span className="min-w-0"><span className="block text-sm font-medium">{step.label}</span><span className="mt-0.5 block truncate text-xs text-[var(--muted-foreground)]">{step.meta}</span></span><ChevronRight className="ml-auto mt-1 size-4 text-slate-300" /></button>)}</div></CardContent></Card></div><div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4"><MetricCard label="学情诊断" value={state.diagnosis ? `${state.diagnosis.score} 分` : "未完成"} hint={state.diagnosis?.level ?? "知识基础与学习特征"} icon={ClipboardCheck} tone="blue" /><MetricCard label="当前项目" value={project ? project.title.slice(0, 9) : "未选择"} hint={project?.level ? `${project.level} 难度` : "来自项目案例库"} icon={Target} tone="green" /><MetricCard label="任务进度" value={`${progress}%`} hint="五阶项目闯关" icon={ListChecks} tone="amber" /><MetricCard label="综合评价" value={reportScore ? `${reportScore} 分` : "待生成"} hint={state.evaluation?.grade ?? "四阶能力增值评价"} icon={FileBarChart} tone="red" /></div><div className="grid gap-4 lg:grid-cols-[1.1fr_.9fr]"><Card className="rounded-lg border-[var(--border)] shadow-none"><CardHeader className="p-6 pb-3"><CardTitle className="text-base">四阶能力目标</CardTitle><CardDescription className="mt-1">每个项目都应留下对应证据</CardDescription></CardHeader><CardContent className="grid gap-3 p-6 pt-3 sm:grid-cols-2">{[{title: "知识理解", text: "识别概念，解释现象，建立联系。", color: "#1e4f91"}, {title: "专业应用", text: "建模计算，实验测量，迁移解决问题。", color: "#2a8a78"}, {title: "创新实践", text: "设计方案，优化系统，形成作品。", color: "#c47a2c"}, {title: "团队协作", text: "表达论证，分工协同，反思改进。", color: "#6c58a5"}].map((item) => <div key={item.title} className="rounded-md bg-slate-50 p-4"><div className="mb-3 size-2 rounded-full" style={{ backgroundColor: item.color }} /><p className="text-sm font-semibold">{item.title}</p><p className="mt-1 text-xs leading-5 text-[var(--muted-foreground)]">{item.text}</p></div>)}</CardContent></Card><Card className="rounded-lg border-[var(--border)] shadow-none"><CardHeader className="p-6 pb-3"><CardTitle className="text-base">开始位置</CardTitle><CardDescription className="mt-1">根据当前状态选择一个入口</CardDescription></CardHeader><CardContent className="space-y-2 p-6 pt-3">{[{label: "补齐基础画像", desc: "前测还未完成", view: "diagnosis" as StudentView, icon: ClipboardCheck}, {label: "寻找匹配项目", desc: "查看章节与案例关联", view: "projects" as StudentView, icon: Library}, {label: "运行一次实验", desc: "记录可复核的数据", view: "lab" as StudentView, icon: FlaskConical}].map((item) => <button key={item.label} onClick={() => navigate(item.view)} className="flex w-full items-center gap-3 rounded-md border border-transparent p-3 text-left transition-colors hover:border-[var(--border)] hover:bg-slate-50"><span className="grid size-8 place-items-center rounded-md bg-blue-50 text-[var(--primary)]"><item.icon className="size-4" /></span><span className="min-w-0"><span className="block text-sm font-medium">{item.label}</span><span className="block text-xs text-[var(--muted-foreground)]">{item.desc}</span></span><ChevronRight className="ml-auto size-4 text-slate-300" /></button>)}</CardContent></Card></div></div>
}
...
function TasksView({ state, update, navigate }: { state: AppState; update: (patch: Partial<AppState> | ((current: AppState) => AppState)) => void; navigate: (view: StudentView) => void }) {
  ...
  return <div className="space-y-5"><div className="flex flex-col justify-between gap-3 rounded-lg border border-blue-200 bg-blue-50/70 p-5 md:flex-row md:items-center"><div><p className="text-xs font-semibold uppercase tracking-[0.14em] text-[var(--primary)]">当前项目</p><h2 className="mt-2 text-lg font-semibold text-[var(--primary-strong)]">{project.title}</h2><p className="mt-1 text-sm text-blue-900/70">{project.ability}</p></div><div className="min-w-[180px]"><div className="mb-2 flex justify-between text-xs text-blue-900/70"><span>总进度</span><strong>{progress.percentage}%</strong></div><Progress value={progress.percentage} className="h-2 bg-blue-100" /></div></div><div className="grid gap-4 lg:grid-cols-[280px_1fr]"><Card className="rounded-lg border-[var(--border)] shadow-none"><CardHeader className="p-5 pb-3"><CardTitle className="text-base">五阶任务</CardTitle><CardDescription className="mt-1">点击切换任务阶段</CardDescription></CardHeader><CardContent className="space-y-1 p-3 pt-2">{TASK_TEMPLATES.map((task, index) => { const taskDone = task.checks.every((_, checkIndex) => state.taskChecks[`${index}-${checkIndex}`]); return <button key={task.id} onClick={() => setActive(index)} className={cn("flex w-full items-start gap-3 rounded-md p-3 text-left transition-colors", active === index ? "bg-blue-50 text-[var(--primary-strong)]" : "hover:bg-slate-50")}><span className={cn("mt-0.5 grid size-6 shrink-0 place-items-center rounded-full border text-xs", taskDone ? "border-emerald-200 bg-emerald-50 text-emerald-700" : active === index ? "border-blue-300 bg-white text-[var(--primary)]" : "border-slate-200 bg-slate-50 text-slate-500")}>{taskDone ? <Check className="size-3" /> : index + 1}</span><span><span className="block text-sm font-medium">{task.name}</span><span className="mt-1 block text-xs text-[var(--muted-foreground)]">{task.ability}</span></span></button> })}</CardContent></Card><Card className="rounded-lg border-[var(--border)] shadow-none"><CardHeader className="p-6 pb-3"><div className="flex flex-wrap items-center gap-2"><StatusBadge tone="info">{current.ability}</StatusBadge><StatusBadge>{active + 1}/5</StatusBadge></div><CardTitle className="mt-3 text-xl">{current.name}</CardTitle><CardDescription className="mt-2 leading-6">{current.goal}</CardDescription></CardHeader><CardContent className="space-y-5 p-6 pt-3"><div><p className="mb-3 text-sm font-semibold">闯关条件</p><div className="space-y-2">{current.checks.map((check, index) => { const key = `${active}-${index}`; return <label key={check} className="flex cursor-pointer items-center gap-3 rounded-md border border-[var(--border)] p-3 text-sm hover:bg-slate-50"><input type="checkbox" checked={Boolean(state.taskChecks[key])} onChange={(event) => toggleCheck(key, event.target.checked)} className="size-4 accent-[var(--primary)]" />{check}</label> })}</div></div><label className="block space-y-2 text-sm font-medium">过程说明<span className="block text-xs font-normal text-[var(--muted-foreground)]">记录思路、证据、困难和下一步计划。</span><Textarea value={text} onChange={(event) => setText(event.target.value)} rows={5} placeholder="例如：我先用……测量……，发现……，下一步准备……" /></label><div className="flex flex-wrap gap-2"><Button onClick={saveTask} className="bg-[var(--primary)] hover:bg-[var(--primary-strong)]"><Save className="size-4" />保存本关记录</Button>{active < TASK_TEMPLATES.length - 1 && <Button variant="outline" onClick={() => { saveTask(); setActive((index) => index + 1) }}>进入下一关<ArrowRight className="size-4" /></Button>}</div><div className="flex items-start gap-3 rounded-md border border-blue-200 bg-blue-50 p-4 text-sm leading-6 text-blue-900"><Bot className="mt-0.5 size-4 shrink-0 text-[var(--primary)]" /><p><strong>AI 实践智能体提示：</strong>{current.prompt}</p></div></CardContent></Card></div></div>
}
...
function AssistantView({ state, update }: { state: AppState; update: (patch: Partial<AppState> | ((current: AppState) => AppState)) => void }) {
  ...
  return <div className="grid gap-4 lg:grid-cols-[250px_1fr]"><Card className="rounded-lg border-[var(--border)] shadow-none"><CardHeader className="p-5 pb-3"><CardTitle className="text-base">快捷问题</CardTitle><CardDescription className="mt-1">从一个具体问题开始</CardDescription></CardHeader><CardContent className="space-y-2 p-4 pt-1">{quickQuestions.map((question) => <button key={question} onClick={() => send(question)} className="w-full rounded-md border border-transparent p-3 text-left text-sm leading-5 transition-colors hover:border-[var(--border)] hover:bg-slate-50">{question}<ChevronRight className="mt-1 inline size-3.5 text-slate-400" /></button>)}<div className="mt-3 rounded-md bg-blue-50 p-3 text-xs leading-5 text-blue-900"><ShieldCheck className="mb-2 size-4 text-[var(--primary)]" /><strong className="block">知识辅导</strong>围绕课程知识提供分步追问、提示和验证建议。</div></CardContent></Card><Card className="flex min-h-[560px] flex-col rounded-lg border-[var(--border)] shadow-none"><CardHeader className="flex-row items-center justify-between border-b border-[var(--border)] p-5"><div><CardTitle className="text-base">知物对话</CardTitle><CardDescription className="mt-1">追问 · 提示 · 验证</CardDescription></div><StatusBadge tone="success">在线</StatusBadge></CardHeader><CardContent className="flex-1 space-y-4 overflow-y-auto p-5">{log.map((message, index) => <div key={`${message.role}-${index}`} className={cn("flex gap-3", message.role === "user" && "justify-end")}><Avatar className={cn("size-8 shrink-0", message.role === "ai" ? "bg-blue-100 text-[var(--primary)]" : "bg-slate-700 text-white") }><AvatarFallback>{message.role === "ai" ? "知" : "我"}</AvatarFallback></Avatar><div className={cn("max-w-[78%] whitespace-pre-line rounded-lg px-4 py-3 text-sm leading-6", message.role === "ai" ? "bg-slate-100 text-slate-700" : "bg-[var(--primary)] text-white")}>{message.text}</div></div>)}</CardContent><CardFooter className="border-t border-[var(--border)] p-4"><div className="flex w-full items-end gap-2"><Textarea value={input} onChange={(event) => setInput(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter" && !event.shiftKey) { event.preventDefault(); send() } }} placeholder="输入物理问题、实验困难或项目思路" rows={2} /><Button aria-label="发送问题" onClick={() => send()} className="size-10 shrink-0 bg-[var(--primary)] p-0 hover:bg-[var(--primary-strong)]"><Send className="size-4" /></Button></div></CardFooter></Card></div>
}
